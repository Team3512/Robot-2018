...
static constexpr double kFreeSpeed = 5310.0; // no load rpm
static constexpr double kFreeCurrent = 2.7; // amps
static constexpr double kme = 6.80389; // estimated mass of elevator (carriage) in kg
static constexpr double kmr = 108.862;// robot mass 120.0lb + another robot mass 120.0lb to kg
...
static constexpr double Kv = (12.0 - kR * kFreeCurrent) / (kFreeSpeed / 60.0 * 2.0 * kPi); // velocity constant
static constexpr double Kt = (kNumMotors * kStallTorque) / kStallCurrent; // torque constant
static constexpr double khighG = (42.0 / 12.0) * (40.0 / 14.0); // high gear-ratio
static constexpr double klowG = (42.0 / 12.0) * (60.0 / 14.0); // low gear-ratio
