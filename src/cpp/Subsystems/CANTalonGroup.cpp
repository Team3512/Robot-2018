...
void CANTalonGroup::Set(double value) {
    if (m_forwardLimit != nullptr && m_reverseLimit != nullptr) {
        ...
        std::cout << "Value: " << value << " ForwardLimit: " << m_forwardLimit->Get() << " ReverseLimit: " << m_reverseLimit->Get() << std::endl;
    }
...
void CANTalonGroup::EnableHardLimits(frc::DigitalInput* forwardLimitSwitch, frc::DigitalInput* reverseLimitSwitch) {
