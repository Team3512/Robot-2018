...
void Climber::HandleEvent(Event event) {
	static State state = State::Idle;
	   ...
	   switch (state) {
	        case State::Init:  // Wait for ButtonPressed
	            ...
	            break;
	        case State::Setup:  // Stow Intake, Raise Elevator
	        ...
	        case State::Aligned:
	        	...
	        	m_elevatorShifter(true);
	    		state = State::PawlEngage;
	            break;

	        case State::PawlEngage:
	        	if (/*appendageStick.GetRawButtonPressed()*/){
	        		m_pawl.Set(true);
	            	state = State::Climb;
	        	}
	            break;
	        case State::Climb:  // Lower Elevator with a possible cancel button

	            if (/* at set point */){
	            	state = State::Idle;
	            }
	        	break;
	        case State::Idle:  // Stop Elevator's Closed Loop
